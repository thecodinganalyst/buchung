...
```bash
my-application$ sam local invoke putItemFunction --event events/event-set-capacity-for-date-location.json
my-application$ sam local invoke getAllItemsFunction --event events/event-get-capacity-by-date-location.json
```
...
```bash
aws cloudformation delete-stack --stack-name buchung
```
